...
// Song Peng: there is some error with texture coordinate if not comment this line
...
glmReadMTL(GLMmodel* model, char* name)
{
	...
	break;
        ...
        case 'm':
			...
			filename = (char *)malloc(FILENAME_MAX); // Modified by Song Peng on 2012.12.02
...
glmDraw(GLMmodel* model, GLuint mode)
{
    ...
    for(blenditer = 0; blenditer<2; blenditer++)
	{
		...
		while (group) 
		{
			// Song Peng 2017.11.13: need to comment this such that textured model can be rendered without crash
			...
			for (i = 0; i < group->numtriangles; i++) 
			{
				...
				if(blending == blenditer) 
				{
					...
					for (j=0; j<3; j++) 
					{
						...
						//	printf("Tex [%.3f %.3f]  W: %.3f  H: %.3f \n", model->texcoords[2 * triangle->tindices[j]], model->texcoords[2 * triangle->tindices[j] + 1],
						//												   model->textures[map_diffuse].width, model->textures[map_diffuse].height);
						//}

						// Song Peng: don't consider map_diffuse
						if (mode & GLM_TEXTURE && (triangle->tindices[j]!=-1) ) 
						{
							glTexCoord2f(model->texcoords[2 * triangle->tindices[j]],model->texcoords[2 * triangle->tindices[j] + 1]);
						}
						//===========================================================//
